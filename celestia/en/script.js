...
if (guestGrid) {
  if (!guestCommunities.length) {
    ...
    empty.textContent = 'Guest communities will be added here.';
    ...
    [...guestCommunities]
      ...
      .forEach((community) => {
        ...
        if (community.vrchat) {
          ...
          vrchat.textContent = 'VRChat Group';
    ...
    joinCard.setAttribute('aria-label', 'This space could be yours - open Discord');
    ...
    joinText.textContent = 'This space could be yours';
...
if (staffGrid) {
  ...
  staffStyles.textContent = `
    ...
    .staff-level[data-level="4"] .staff-level__grid{gap:12px}
    .staff-level[data-level="4"] .staff-card{flex:0 1 190px;width:min(100%,190px)}
    .staff-level[data-level="4"] .staff-card__media{width:104px;height:104px;margin-top:18px}
    .staff-level[data-level="4"] .staff-card__info{padding:16px 14px 18px}
    .staff-level[data-level="4"] .staff-card__info h3{font-size:1rem}
    .staff-level[data-level="4"] .staff-card__role{font-size:.74rem}
    ...
    @media(max-width:620px){.staff-level + .staff-level{margin-top:28px;padding-top:28px}.staff-level__grid .staff-card{flex-basis:100%;width:100%}.staff-level[data-level="4"] .staff-card{flex:0 1 calc(50% - 6px);width:calc(50% - 6px)}.staff-level[data-level="4"] .staff-card__media{width:86px;height:86px}.staff-divider{margin:34px 0 28px}.staff-level__grid .staff-card__media{width:144px;height:144px}}
  `;
  ...
  orderedLevels.forEach((level) => {
    ...
    if (level >= 5 && !collaboratorsDividerAdded) {
      ...
      divider.className = 'staff-divider';
      const label = document.createElement('span');
      label.textContent = 'External collaborators';
      divider.appendChild(label);
      fragment.appendChild(divider);
...
if (heroSection && !document.getElementById('trailer')) {
  ...
  head.innerHTML = `
    ...
    <p>Get a first look at the new version of Celestia and get ready to return among the stars.</p>
  `;
  ...
  if (trailerVideoId) {
    ...
    iframe.title = 'Celestia Remastered Trailer';
    iframe.loading = 'lazy';
    ...
    placeholder.innerHTML = `
      <div class="trailer-placeholder__inner">
        ...
        <p>The YouTube player is already set up. It will be enabled as soon as the official trailer link is available.</p>
      </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (label) label.textContent = 'Founders (not staff)';
});